...
export function CodeGameCard({ post }: CodeGameCardProps) {
  ...
  return (
    <div className="group flex flex-col justify-between overflow-hidden rounded-2xl border border-indigo-950/10 bg-white shadow-sm transition-all duration-200 hover:-translate-y-1 hover:border-sapphire-600/30 hover:shadow-xl">
      <Link 
        to={`/post/${post.slug}`}
        className="block"
      >
      ...
      {/* Instant 1-Click Code Copy Box */}
      {topCode && (
        <div className="mx-5 mb-4 p-2.5 rounded-xl bg-azure-50/80 border border-indigo-950/10 flex items-center justify-between gap-2">
          <div className="min-w-0 flex-1">
            <div className="text-[10px] font-black uppercase text-indigo-900/50 tracking-wider flex items-center gap-1">
              <Zap size={11} className="text-amber-500" /> Top Code
            </div>
            <div className="font-mono text-xs sm:text-sm font-black text-indigo-950 truncate select-all">
              {topCode.code}
            </div>
          </div>
      ...
      <Link 
        to={`/post/${post.slug}`}
        className="border-t border-indigo-950/5 bg-azure-50/40 px-5 py-3.5 flex items-center justify-between group-hover:bg-azure-100/50 transition-colors"
      >
        <span className="text-xs font-black text-sapphire-700 uppercase tracking-wider">
          View All {activeCodes.length} Codes & Guide
        </span>
